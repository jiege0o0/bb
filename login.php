<?php 
	...
	if($quick_password)
		$sql = "select * from user where name='".$username."' and quick_password='".$quick_password."'";
	else
	...
	if($mainUserData)//可以登录
	{
		...
		$sql = "update user set last_land=".$time.",quick_password='".$quick_password."' where id='".$mainUserData['id']."'";
		if(!$conne->uidRst($sql))
		{
			$returnData->fail = 2;
		}
